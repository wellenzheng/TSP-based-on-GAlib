...
#define TSP_TEST1 "berlin52.txt" //7542
#define TSP_TEST2 "rat99.txt"    //1211
#define TSP_TEST3 "lin105.txt"   //14379
#define TSP_TEST4 "pr76.txt"     //108159
...
//辅助函数
int mod(double a, double b);                                  //取余函数
void swap(int &a, int &b);                                    //交换
void shift(GA1DArrayGenome<int> &parent, int start, int end); //用于HGACrossover中右移操作
int findGene(GA1DArrayGenome<int> &g, int gene);              //查找某个特定基因的位置
...
int main()
{
  ...
  double CityID;
  ifstream in(TSP_TEST1);
  if (!in)
  {
    cerr << "could not read data file " << TSP_TEST1 << "\n";
    exit(1);
  }
  int ntowns = 0;
  do
...
int HGACrossover(const GAGenome &g1, const GAGenome &g2, GAGenome *c1, GAGenome *c2)
{
  ...
  int genome = parent1.gene(GARandomInt(0, genomelength - 1));
  int curr = 0;

  while (curr < genomelength)
  {
    child1.gene(curr, genome);
    visit1[child1.gene(curr)] = true;
    int index1 = findGene(parent1, genome);
    int index2 = findGene(parent2, genome);
    int dis1 = DISTANCE[parent1.gene(index1)][parent1.gene(mod(index1 + 1, genomelength))];
    int dis2 = DISTANCE[parent2.gene(index2)][parent2.gene(mod(index2 + 1, genomelength))];
    if (dis1 < dis2)
    {
      genome = parent1.gene(mod(index1 + 1, genomelength));
    }
    else
    {
      genome = parent2.gene(mod(index2 + 1, genomelength));
    }
    curr++;
  }

  genome = parent2.gene(GARandomInt(0, genomelength - 1));
  curr = 0;
  ...
  while (curr < genomelength)
  {
    ...
    int index1 = findGene(parent1, genome);
    int index2 = findGene(parent2, genome);
    while (visit1[parent1.gene(mod(index1 + 1, genomelength))])
    {
      index1 = GARandomInt(0, genomelength - 1);
    }
    while (visit1[parent2.gene(mod(index1 + 1, genomelength))])
    {
      index2 = GARandomInt(0, genomelength - 1);
    }
