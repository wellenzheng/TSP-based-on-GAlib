...
int PositionBasedCrossover(const GAGenome &g1, const GAGenome &g2, GAGenome *c1, GAGenome *c2)
{
  ...
  {
    indeice.push_back(GARandomInt(0, genomelength - 1));
  }
  ...
  for (int i = 0; i < indeice.size(); i++)
  {
    ...
    visitCity1[parent2.gene(i)] = true;
    child2.gene(i, parent1.gene(i));
    visitIndex2[indeice[i]] = true;
    visitCity2[parent1.gene(i)] = true;
  }
  ...
  for (int i = 0, j = 0; i < genomelength; i++)
  {
    if (!visitIndex1[i])
    {
      while (visitCity1[parent1.gene(j)])
      {
        j++;
      }
      child1.gene(i, parent1.gene(j));
    }
  }
  ...
  for (int i = 0, j = 0; i < genomelength; i++)
  {
    if (!visitIndex2[i])
    {
      while (visitCity2[parent2.gene(j)])
      {
        j++;
      }
      child2.gene(i, parent2.gene(j));
    }
  }
  //////
...
int CycleCrossover(const GAGenome &g1, const GAGenome &g2, GAGenome *c1, GAGenome *c2)
{
  ...
  child1.gene(first, firstGene);
  visit1[firstGene] = true;
  ...
  //child2
  first = GARandomInt(0, genomelength - 1);
  firstGene = parent2.gene(first);
  lastGene = parent1.gene(first);
  child2.gene(first, firstGene);
  visit2[firstGene] = true;
  while (firstGene != lastGene)
  {
    for (int i = 0; i < genomelength; i++)
    {
      if (lastGene == parent2.gene(i))
      {
...
int SubtourExchangeCrossover(const GAGenome &g1, const GAGenome &g2, GAGenome *c1, GAGenome *c2)
{
  ...
  for (int i = 0; i < number; i++)
  {
    int r = GARandomInt(0, genomelength - 1);
    visit1[parent1.gene(r)] = true;
    visit2[parent1.gene(r)] = true;
  }
  ...
  while (number--)
  {
    while (!visit1[parent1.gene(i++)])
      ;
    while (!visit2[parent2.gene(j++)])
      ;
...
int HGACrossover(const GAGenome &g1, const GAGenome &g2, GAGenome *c1, GAGenome *c2)
{
  ...
  {
    int gene;
    if (DISTANCE[parent1.gene(left)][parent1.gene(mod(left + 1, genomelength))] < DISTANCE[parent2.gene(left)][parent2.gene(mod(left + 1, genomelength))])
    {
