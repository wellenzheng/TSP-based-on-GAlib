...
GA3DBinaryStringGenome::resizeBehaviour(GAGenome::Dimension which) const {
  ...
  if(which == WIDTH) {
    ...
    if(maxY == minY) val = GAGenome::GENOME_FIXED_SIZE;
    else val = maxY;
  }
  else if(which == DEPTH) {
    if(maxZ == minZ) val = GAGenome::GENOME_FIXED_SIZE;
    else val = maxZ;
  }
  return val;
}
...
GA3DBinaryStringGenome::UniformInitializer(GAGenome & c)
{
  ...
  child.resize(GAGenome::GENOME_ANY_SIZE,GAGenome::GENOME_ANY_SIZE,GAGenome::GENOME_ANY_SIZE);
...
OnePointCrossover(const GAGenome& p1, const GAGenome& p2,
  ...
  if(c1 && c2){
    ...
    if(sis.resizeBehaviour(GAGenome::WIDTH) == GAGenome::GENOME_FIXED_SIZE &&
       bro.resizeBehaviour(GAGenome::WIDTH) == GAGenome::GENOME_FIXED_SIZE){
    ...
    else if(sis.resizeBehaviour(GAGenome::WIDTH) == GAGenome::GENOME_FIXED_SIZE ||
	    bro.resizeBehaviour(GAGenome::WIDTH) == GAGenome::GENOME_FIXED_SIZE){
    ...
    if(sis.resizeBehaviour(GAGenome::HEIGHT) == GAGenome::GENOME_FIXED_SIZE &&
       bro.resizeBehaviour(GAGenome::HEIGHT) == GAGenome::GENOME_FIXED_SIZE){
    ...
    else if(sis.resizeBehaviour(GAGenome::HEIGHT) == GAGenome::GENOME_FIXED_SIZE ||
	    bro.resizeBehaviour(GAGenome::HEIGHT) == GAGenome::GENOME_FIXED_SIZE){
      GAErr(GA_LOC, mom.className(), "one-point cross", gaErrSameBehavReqd);
      return nc;
    }
    ...
    if(sis.resizeBehaviour(GAGenome::DEPTH) == GAGenome::GENOME_FIXED_SIZE &&
       bro.resizeBehaviour(GAGenome::DEPTH) == GAGenome::GENOME_FIXED_SIZE){
    ...
    else if(sis.resizeBehaviour(GAGenome::DEPTH) == GAGenome::GENOME_FIXED_SIZE ||
	    bro.resizeBehaviour(GAGenome::DEPTH) == GAGenome::GENOME_FIXED_SIZE){
      GAErr(GA_LOC, mom.className(), "one-point cross", gaErrSameBehavReqd);
      return nc;
    }
